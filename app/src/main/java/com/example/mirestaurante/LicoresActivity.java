...
public class LicoresActivity extends AppCompatActivity {
    ...
    private LicoresActivity.MyAdapterL myAdapter = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        myAdapter = new LicoresActivity.MyAdapterL(this);
    ...
    public void setData( ) {
        ...
        String[] licoresNombres = getResources().getStringArray(R.array.licores);
        String[] licoresDescripciones = getResources().getStringArray(R.array.licoresDescripciones);

        TypedArray licoresImagenes = getResources().obtainTypedArray(R.array.licoresImagenes);

        // Iterar sobre las categorías y crear los objetos Producto
        for (int i = 0; i < licoresNombres.length; i++) {
            Producto producto = new Producto(
                    licoresNombres[i],  // Título
                    licoresDescripciones[i],  // Descripción
                    licoresImagenes.getResourceId(i, -1)
            );
            l_categorias.add(producto);
        }
    }

    public static class MyAdapterL extends BaseAdapter {

        private Context mContext;
        public MyAdapterL(Context c) {
            mContext = c;
        }
